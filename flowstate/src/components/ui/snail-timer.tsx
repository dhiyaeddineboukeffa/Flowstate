...
export const SnailTimer = ({
  ...
  return (
    <div
      className={`snail-timer ${started ? "" : "is-paused"}`}
      style={{
        "--snail-move-duration": `${initialSeconds}s`,
        "--snail-move-delay": `-${initialElapsed}s`,
        "--snail-eye-duration": "1s",
        "--snail-scale-duration": "1s",
      } as React.CSSProperties}
    >
      <div className={`snail-timer__track ${isBreak ? 'snail-timer__track--break' : 'snail-timer__track--work'}`}>
        <div className="flex items-end">
          <div className="snail">
            <SnailSvg isBreak={isBreak} />
          </div>
          <div className="snail-timer__dust" aria-hidden="true">
            <DustSvg isBreak={isBreak} />
          </div>
        </div>
      </div>
    </div>
...
const SnailSvg = ({ isBreak }: { isBreak: boolean }) => {
  const primaryColor = isBreak ? "#60A5FA" : "#9A70EA";
  const darkColor = isBreak ? "#3B82F6" : "#7247FF";
...
const DustSvg = ({ isBreak }: { isBreak: boolean }) => {
  const primaryColor = isBreak ? "#60A5FA" : "#9A70EA";
  return (
    <svg width="40" height="20" viewBox="0 0 40 20" className="opacity-60" style={{ fill: primaryColor }}>
